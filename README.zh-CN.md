...
评估流程支持交付物为媒体文件的技能，而不只是文本。一次运行产生的每个文件都会被收集
并计算哈希作为 artifact。确定性检查对其进行测量（draw.io XML 结构、图片尺寸、帧数与
时长、对照参考图的几何位置）；在 `full` 配置下，视觉 judge 还会根据请求审阅渲染出的
PNG、WebP 或 MP4 帧。这里的三个技能中有两个输出图片或视频。
